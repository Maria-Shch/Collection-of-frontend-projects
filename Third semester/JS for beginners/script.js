...
function Stopwatch(elem) {
  ...
  function update() {
    ...
    elem.textContent = timeHandler(time2); 
  }

  function timeHandler(time2) {
    time2 = new Date(time2);

    let minutes = time2.getMinutes().toString();
    let seconds = time2.getSeconds().toString();
    let milliseconds = time2.getMilliseconds().toString().slice(0,-1);// slice(0,-1) - отбрасывает последнее значение от милисекунд 
